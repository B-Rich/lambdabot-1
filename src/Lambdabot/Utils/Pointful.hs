...
module Lambdabot.Utils.Pointful (pointful, ParseResult(..), test, main, combinatorModule) where
...
succName _ = error "HsIdent expected"
...
combinators = M.fromList $ map declToTuple defs
  where defs = case parseModule combinatorModule of
        ...
        declToTuple _ = error "Can't conver declaration to tuple: incorrect pattern bindings"
